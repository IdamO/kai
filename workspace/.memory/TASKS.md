...
- [~] **Activation-quality calibration** — PARTIAL 2026-04-21 17:20 PT. Raw battery v2 JSON (/tmp) cleared; re-run requires $5-10 spend-gate approval from Idam. Computed coarse-grained PASS/FAIL aggregate as substitute: see files/consultations/2026-04-17-brain-memory-followup/activation-quality-calibration-partial.md. Single decisive brain-layer win: adv3_reversal_chain (new_brain PASS, both old conditions FAIL truncated). Full Spearman still blocked on spend approval.
- [x] **Atomizer noise fix** — SHIPPED 2026-04-21 16:50 PT. Guard rewrite in `src/kai/brain/atomize.py` (commit bff4ef59): Path 2 now requires either strong topic in overlap OR ≥2 substantive content tokens matching in both headings (after stripping [CATEGORY] prefixes + generic stop words). Impact on current index: 493 → 412 supersede edges (81 pruned, 16.4% reduction). 7/7 unit tests pass. False-positive chains (Full Email Catch-Up, 3-Day Catch-Up) fully removed. Real revisions (birthday v4.1→v4.2, MODEL-002 updates) preserved.
